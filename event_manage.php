	...
	<div id="add">
		<form action="event_manage.php#add" method="POST">
			<table align="center" cellspacing="20px">
				...
				<tr><td>Guest Lecturer: 
					<select name="a_guestname" >
						<?php
							$conn = mysqli_connect($servername, $username, $password, $dbname);
							$read_usertype = "SELECT * FROM user_details WHERE UserType='Guest' ";
							$result_read_usertype = mysqli_query($conn, $read_usertype);
							if(mysqli_num_rows($result_read_usertype)>0){
								while($row = mysqli_fetch_array($result_read_usertype, MYSQLI_ASSOC)){
		...
		if (isset($_POST['addevent'])) {
			...
			if($result_read_venue_id){
				while($row = mysqli_fetch_array($result_read_venue_id, MYSQLI_ASSOC)){
					$vid=$row['VenueID'];
					//Insert event
					$insert_event = "INSERT INTO event_details (EventName, EventDate, EventTime, EventCategory, EventDescription, EventTicketPrice, EventTicketTotal, VenueID,GuestID, UserID) VALUES ('$ename', '$edate', '$etime', '$ecategory', '$edescription', $eprice, $etotal, '$vid','$eguest','$uid')";
